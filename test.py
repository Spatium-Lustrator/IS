...
def pics(message):
    global message_id
    all_data1['mesid'] = qb.send_message(message.from_user.id, 'Прив')

@qb.message_handler(content_types=['text'])
def aa(message):
    if message.text == '1':
        qb.edit_message_text(text='2', chat_id=message.chat.id, message_id=all_data1['mesid'])
